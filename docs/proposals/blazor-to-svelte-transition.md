...
Bootstrap 5 + Font Awesome assets continue to be built by the existing Yarn pipeline; the SvelteKit project consumes the compiled CSS rather than re-deciding the design system. Material Design principles (elevation, motion, typography) are applied within Svelte components per the Frontend Expert standards.
...
| Bootstrap / Font Awesome pipeline disruption                                               | The existing Yarn 4 + corepack pipeline in `src/Heimdall.Web/` is unchanged. The compiled CSS is imported by the SvelteKit project's root layout; no design-system decision is reopened.                                                                                          |
...
**Next step:** Review and resolve the open questions in §11, confirm the Phase 6 insertion in [`security-and-authorization.md`](./security-and-authorization.md) §9.3, then schedule step 1 of §9 (SvelteKit scaffold) for *after* Phase 5 merges.
