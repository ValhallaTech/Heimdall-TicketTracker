...
| 2026-05-07 | Phase 3.1 + 3.2 landed: `authz/model.fga` authored with `fga model test` passing on `authz/store.fga.yaml`; `render.yaml` extended with the `heimdall-ticket-tracker-openfga` private service (preshared-key auth, `openfga migrate` pre-deploy on the `heimdall_authz` logical DB) and `OPENFGA_API_URL` / `OPENFGA_PRESHARED_KEY` / `OPENFGA_STORE_ID` / `OPENFGA_AUTHORIZATION_MODEL_ID` wired into the web service; bootstrap published as `scripts/openfga-bootstrap.sh` + [`docs/runbooks/openfga-bootstrap.md`](../runbooks/openfga-bootstrap.md). Phase 3.3 (SDK + adapter) is next. |
| 2026-05-08 | Phase 3.3 + 3.4 landed (steps 5–8). `OpenFga.Sdk` + `OpenTelemetry.Api` + `Microsoft.Extensions.Caching.Memory` referenced from `Heimdall.BLL`. Typed `OpenFgaOptions` bound from `Authorization:OpenFga` with env-var post-bind precedence (`OPENFGA_API_URL` / `OPENFGA_STORE_ID` / `OPENFGA_AUTHORIZATION_MODEL_ID` / `OPENFGA_PRESHARED_KEY`); `AuthorizationModelId` pinned globally on `ClientConfiguration`. `IOpenFgaAuthorizationService` adapter (named with the `OpenFga` prefix to disambiguate from `Microsoft.AspNetCore.Authorization.IAuthorizationService`) wraps `Check` / `BatchCheck` / `ListObjects`, exposes `FgaConsistency`, caches via process-wide `IMemoryCache` (cache key includes the consistency level so a `HigherConsistency` lookup can never be served from a `MinimizeLatency` entry; deny-closed on every non-cancellation exception). `ITupleWriter` seam emits a single OpenFGA `Write` per call (atomic writes+deletes), with `ReplaceAsync(delete?, write?)` for assignee transitions. Hooks fire after the DB commit on `TicketService.CreateAsync` (`parent_project` + `reporter` + optional `assignee`), `TicketService.AssignTicketAsync`/`ClaimTicketAsync` (atomic `ReplaceAsync`), and `DefaultHierarchyBootstrapper` (always-emit seed hierarchy + admin tuples). `OpenFgaBackfillJob` paginates the entire DB at ≤ 100 tuples/`Write` and emits one `openfga_backfill_completed` audit row; `OpenFgaBackfillRunner` is gated on `HEIMDALL_OPENFGA_BACKFILL=1`. Two openfga-expert review fixes: (a) `OpenFgaTupleWriter` catches `FgaApiValidationError` with `code = "write_failed_due_to_invalid_input"` and treats it as idempotent (Debug log + duplicate counter, no audit row) so backfill / always-emit re-runs are silent; (b) the per-call `Activity` no longer carries the high-cardinality `object` tag (kept `object_type` only). When any of `ApiUrl` / `StoreId` / `ModelId` is unset, DI auto-falls-back to `NoOpOpenFgaAuthorizationService` + `NoOpTupleWriter` so unit and acceptance tests boot without a sidecar; production is guarded by `OpenFgaHealthProbe` (gated on `Authorization:OpenFga:HealthProbeEnabled`) which calls `ReadAuthorizationModel` against the pinned model id and aborts startup on failure. `Authorization:Provider` remains `"TeamRole"` — cutover (step 9) is a separate PR. |
| 2026-05-08 | Phase 3.5 landed (steps 9 + 10). Cutover from "authenticated-only" Phase 1 gates to named, OpenFGA-resolved `[Authorize]` policies. Twelve policies registered in `AuthorizationConfiguration` and `AuthorizationPolicies` constants (`CanViewOrganization`, `CanManageOrganizationMembers`, `CanViewTeam`, `CanManageTeamMembers`, `CanViewProject`, `CanEditProject`, `CanManageProjectMembers`, `CanViewTicket`, `CanEditTicket`, `CanCommentTicket`, `CanAssignTicket`, `SystemAdmin`); each non-admin policy carries an `OpenFgaRequirement` describing the FGA `(objectType, relation, routeValueKey)` triple. `OpenFgaAuthorizationHandler` resolves the route value (route data first, query string fallback), pulls the principal's `Heimdall.UserId`, fans out to `IOpenFgaAuthorizationService.CheckAsync` with `MinimizeLatency`, and is **deny-closed**: any unresolved id, missing principal, or sidecar fault denies. `SystemAdminAuthorizationHandler` reads `HeimdallUser.system_admin` via `IUserLookup`. Both handlers honour the **DB-only break-glass** path (`HEIMDALL_AUTHZ_BREAK_GLASS=1` *plus* a `HeimdallUser.system_admin == true` check that does not depend on OpenFGA) and emit one `authz_break_glass` audit row per use; a failed audit-write denies. `OperationCanceledException` propagates per the FGA-adapter contract. `OpenFgaPermissionService` ships behind the existing `IPermissionService` seam and fans out via `IOpenFgaAuthorizationService` (`view` / `assign` / `manage_members` / `route` via project `admin`); `ApplicationModule` factory now dispatches `Authorization:Provider == "OpenFga"` → real service, `"TeamRole"` (default) → existing role service, anything else → `NotImplementedPermissionService` (fail-loud). The four admin pages (`AuditFeed`, `Memberships`, `Hierarchy`, `Queues`) replaced their inline `AdminGate` plumbing with `[Authorize(Policy = SystemAdmin)]` and `AdminGate.cs` was deleted. `Routes.razor`'s `<NotAuthorized>` branch now distinguishes authenticated-but-unauthorized (→ `/access-denied` via the new `RedirectToAccessDenied` component) from anonymous (→ `<RedirectToLogin />`) so authenticated non-admins do not redirect-loop. **Deviations** captured in TODO comments: (i) `TicketEdit.razor` is dual-routed `/tickets/new` + `/tickets/{Id:int}` so a single page-level `CanViewTicket` policy can't satisfy both — existing inline `ITicketService` gates remain authoritative pending an openfga-expert decision on splitting the page; (ii) `Tickets.razor` (list view) keeps the global authenticated fallback because per-row gating wants a `ListObjects`-backed query (Phase 3.7 step 13); (iii) `Teams/Queue.razor` is keyed by `Slug` not `teamId` so route-bound `CanViewTeam` would not resolve — the inline `IPermissionService.CanViewTeamQueueAsync` gate fans out through OpenFGA via `OpenFgaPermissionService` and is intentionally retained. **Open questions** (raised back to the openfga-expert): (a) self-claim parity — `OpenFgaPermissionService.CanAssignTicketAsync` carves out an actor-only self-claim path because the FGA model's `assign` relation is `reporter or assignee or admin from parent_project` and excludes plain project members; consider adding a `self_claim` relation. Phase 3.7 step 13 (perf / `ListObjects`) and step 14 (decommission of `TeamRoleBackedPermissionService` + Phase 1 fallback policy) remain pending. |
| 2026-05-09 | Phase 3.6 landed (step 11). The OpenFGA adapter (`IOpenFgaAuthorizationService`) gained two operations: `ListUsersAsync` (filtered to `user:`-typed subjects, returning bare user-ids; deny-closed on transport failure; new OTel histograms `heimdall.openfga.list_users.{latency_ms,results,errors}`) and `ExpandAsync` (forced `MinimizeLatency` per the proposal note that Expand is a sub-second tree walk; new POCO tree types `FgaExpandResult` / `FgaExpandNode` / `FgaExpandLeaf` / `FgaExpandTupleToUserset` / `FgaExpandDifference` so the UI never imports the SDK; new histograms `heimdall.openfga.expand.{latency_ms,errors}`). `NoOpOpenFgaAuthorizationService` returns empty list / empty tree so test stacks without a sidecar continue to boot. `IUserLookup.SearchByEmailAsync(fragment, limit)` added — Postgres `ILIKE` with explicit `%` / `_` / `\\` escaping and a 100-row server-side cap so a hung admin search cannot scan the table. New `IMembershipAdminService` (BLL) wraps the three existing member repositories and routes admin add / remove through the existing `ITupleWriter` seam — DB row, then `membership_added` / `membership_removed` audit row, then tuple write — preserving the dual-write failure-mode contract from §3 step 7 (audit + log + swallow + backfill reconcile). Wire-format roles for org / project are normalised in service before reaching `TupleShapes`; team picker collapses three options (`admin`, `member`, `viewer`) onto `TeamMemberRole.Manager` / `Member` / `Viewer`. Three Razor pages: (a) `Memberships.razor` rebuilt from the placeholder as a 3-tab roster (org / team / project) with debounced (250 ms) email-search add form, modal-confirmed remove, friendly Postgres `23505` → "already a member" surface, and `aria-live="polite"` status region; (b) new `/admin/access` index with a numeric ticket-id input; (c) new `/admin/access/ticket/{TicketId:int}` page that picks a relation (view / comment / edit / assign / delete), calls `ListUsersAsync` (capped at 50 displayed with a `Truncated` badge), resolves user ids → email via `IUserLookup`, then walks the `ExpandAsync` tree as nested `<details>`. `AdminNav.razor` gained the new `access` entry; `IMembershipAdminService` is registered `InstancePerLifetimeScope` in `ApplicationModule.cs`. Activity tags continue to omit the high-cardinality `object` id (relation / object_type / consistency / outcome only) per the 2026-05-08 decision. Write-side ad-hoc tuple grants and ticket-level access page (with ListUsers + Expand for non-ticket objects) remain a follow-up. |
