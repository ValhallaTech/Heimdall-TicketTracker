...
6. **`IEmailSender` abstraction in `Heimdall.Core` + two implementations** registered by configuration: `MailKitEmailSender` (production, built on **[MailKit](https://github.com/jstedfast/MailKit)** for SMTP transport and **[MimeKit](https://github.com/jstedfast/MimeKit)** for message construction — chosen for full RFC compliance, async/cancellation support, and active maintenance, where `System.Net.Mail.SmtpClient` is officially [not recommended for new development](https://learn.microsoft.com/dotnet/api/system.net.mail.smtpclient)) and `NoOpEmailSender` (local dev / SMTP-not-yet-provisioned environments). Both implementations live in `Heimdall.BLL` (or a new `Heimdall.BLL.Email` folder); the `IEmailSender` interface and the `MimeMessage`-free DTO it accepts live in `Heimdall.Core` so `Heimdall.Core` does not take a transitive dependency on MimeKit. **No flow depends on this yet** — it's just the seam, deliberately introduced before any user-visible flow needs it.
...
10. **Password reset + self-service register pages**, **gated on `IEmailSender` resolving to `MailKitEmailSender` rather than `NoOpEmailSender`** (a feature flag evaluated at startup based on configuration). Keeps Phase 1 independently shippable to environments where MailKit's SMTP target hasn't been provisioned yet.
...
4. Policy: require MFA for users matching `Check(organization:<seed-org-uuid>#admin@user:X)` (since OpenFGA precedes this phase in the new order, the admin predicate is already a tuple lookup, not a column read). **Use the stable UUID of the seed org**, *not* its slug — [`team-collaboration.md`](./team-collaboration.md) §5 Q4 explicitly allows the `heimdall` slug to be renamed by an admin, so a slug-keyed policy could silently stop applying after a rename. The UUID is captured into a config setting at bootstrap time alongside `HEIMDALL_BOOTSTRAP_ADMIN_EMAIL`.
...
**Status: Approved (2026-05-15).** Replace the Blazor Server UI with a Svelte 5 (runes) + SvelteKit 2 frontend on `adapter-node`. Full design, topology analysis, page inventory, and decision log live in [`blazor-to-svelte-transition.md`](./blazor-to-svelte-transition.md); the authoritative ordered implementation plan lives in [`docs/implementation/phase-6-checklist.md`](../implementation/phase-6-checklist.md). Depends on **Phase 3 (OpenFGA)** and **Phase 5 (API + tokens)** being fully merged on `main` — Phase 5.1 (signing keys + JWKS) is complete; Phase 5.2–5.7 continue on Blazor before any Phase 6 step begins, and cutover starts only after Phase 5.7 sign-off. Approved design choices: `adapter-node` + Topology B (SvelteKit as external ingress, `Heimdall.Web` reduced to an API); **single-container deployment** — one Docker image, one Render service: the SvelteKit `adapter-node` process handles all external traffic and communicates with `Heimdall.Web` via an internal loopback channel; the browser therefore talks to a **single origin** (the SvelteKit endpoint) and never directly addresses the API, so CORS between the browser and `Heimdall.Web` is not required — CORS headers on `Heimdall.Web` apply only to internal server-to-server traffic; existing `Dockerfile` and `render.yaml` are updated in-place rather than split into two Render services; SvelteKit's default hybrid rendering (initial-load SSR → hydration → CSR for subsequent navigation), with no per-route overrides at cutover; English-only at cutover, no i18n library adopted; **`bits-ui` + `shadcn-svelte`** as the component layer, with Bootstrap 5 / Font Awesome deprecated within the SvelteKit project and removed at the Blazor retirement step (the Razor host retains them until then), and **Tailwind CSS arriving as a transitive design-system dependency** via `shadcn-svelte`; Vitest spec authorship owned by the JavaScript/TypeScript Unit Test Engineer agent (`agents/javascript-unit-tests.md`), while the Frontend Expert authors production Svelte/SvelteKit code only; bUnit retires **one-for-one** in the same PR that lands each Razor page's Svelte equivalent and its Vitest spec. Frontend Expert agent is wired to the official Svelte MCP server (`https://mcp.svelte.dev/mcp`); `eslint-plugin-svelte` and `prettier-plugin-svelte` extend the existing flat `eslint.config.cjs` / `.prettierrc.json` in `src/Heimdall.Web/`. Playwright is added as a net-new end-to-end layer.
...
| 2026-05-15 | **Phase 6 approved.** Blazor → Svelte/SvelteKit migration ([`blazor-to-svelte-transition.md`](./blazor-to-svelte-transition.md)) approved with all six open questions resolved: `adapter-node` retained; single-container deployment; SvelteKit default hybrid rendering (SSR initial-load → hydration → CSR navigation); English-only at cutover; `bits-ui` + `shadcn-svelte` adopted (Bootstrap 5 / Font Awesome deprecated within the SvelteKit project at the retirement step; Tailwind CSS enters as a transitive design-system dependency); Vitest spec authorship owned by the JS/TS Unit Test Engineer agent. Implementation checklist: [`phase-6-checklist.md`](../implementation/phase-6-checklist.md). Phase 5.1 is complete on `main`; Phase 5.2–5.7 continue on Blazor before Phase 6 begins. |
