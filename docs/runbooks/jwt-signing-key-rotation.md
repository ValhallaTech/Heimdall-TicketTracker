...
The very first deploy of a Phase 5 build has zero rows in `signing_keys`. Until a row exists, `ISigningKeyService.GetCurrentSigningKeyAsync()` throws `InvalidOperationException("No active signing key.")` and `POST /api/v1/auth/token` returns `500`. The intended seam is the `HEIMDALL_JWT_BOOTSTRAP=1` environment variable, mirroring the Phase 1 step 8 `HEIMDALL_BOOTSTRAP_ADMIN_EMAIL` pattern:
...
2. **Generate the new key.** Invoke `ISigningKeyService.GenerateAsync(SigningAlgorithm.RS256, TokenOptions.SigningKeyValidity)`. The service:
   ...
   - returns the new `kid`.
   The `AFTER INSERT` trigger writes one `token.signing_key.generated` row to `audit_events`; the issuer's "new key becomes the current signing key" transition emits `token.signing_key.rotated` in the same transaction.
...
6. **Post-rotation audit.** A successful rotation produces, in order, audit rows of types `token.signing_key.generated` → `token.signing_key.rotated` → `token.signing_key.retired`. Search `audit_events` for the rotation timestamp and confirm all three are present and correlate to the same `kid` pair.
...
- **Process-internal cache.** The endpoint caches its assembled response in `IMemoryCache` with a 5-minute absolute expiration, keyed by a tag derived from the current key-ring state. `SigningKeyService.GenerateAsync` and `SigningKeyService.RetireAsync` both call `IJwksCacheInvalidator.Invalidate()` on success, so the in-process cache observes rotations **immediately** rather than after the TTL expires. This is the seam that keeps rotation visible to in-process callers (e.g. the JwtBearer middleware's `IssuerSigningKeyResolver`, which never hits the HTTP endpoint).
...
| Rotation throws `InvalidOperationException` mentioning overlap | Outgoing key's `not_after` is too close to `now()` | Extend `not_after` via `UPDATE signing_keys SET not_after = ... WHERE kid = '<outgoing>'`; re-run [Step 2](#step-2--90-day-rotation-procedure). |
