...
/// <summary>
/// Domain entity representing a user's membership in an <see cref="Organization"/>.
/// Mirrors the <c>organization_members</c> table created by
/// <c>M202605050013_CreateOrganizationMembers</c>. The composite primary key is
/// <c>(user_id, organization_id)</c>. The <see cref="Role"/> property carries the
/// wire-format role string (one of <c>owner</c>, <c>admin</c>, <c>member</c>,
/// <c>viewer</c>); Phase 2 has no in-process consumer of the role, OpenFGA reads it
/// directly in Phase 3.
/// </summary>
