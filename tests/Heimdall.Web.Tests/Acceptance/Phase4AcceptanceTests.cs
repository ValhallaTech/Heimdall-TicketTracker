...
/// </para>
/// <para>
/// <b>Audit-event naming.</b> The Phase 4.7 step 22 checklist mentions the
/// underscore-separated names <c>mfa_challenge_succeeded</c>,
/// <c>mfa_challenge_failed</c>, and <c>mfa_recovery_codes_regenerated</c>,
/// but the production code in <c>src/Heimdall.Web/Endpoints/AccountEndpoints.cs</c>
/// emits dot-separated event types
/// (<c>mfa.challenge.succeeded</c> / <c>mfa.challenge.failed</c> /
/// <c>mfa.recovery_codes.regenerated</c>). The assertions below intentionally
/// match the strings the production code actually writes — bringing the
/// checklist text in line with the wire format is a documentation-only fix
...
public sealed class Phase4AcceptanceTests : IClassFixture<HeimdallWebApplicationFactoryWithOpenFga>
{
    ...
    public async Task Should_DriveFullAdminMfaLifecycle_When_AdminEnrolsChallengesAndDisables()
    {
        ...
        await PostChallengeAsync(mfaClient, code: "000000", expectSuccess: false)
            .ConfigureAwait(false);
    ...
    /// <summary>
    /// Resolves a fresh <see cref="UserManager{T}"/> scope, rotates the user's
    /// authenticator key if it is unset, and computes the current TOTP code
    /// via <see cref="UserManager{T}.GenerateTwoFactorTokenAsync"/>. The
    /// authenticator token provider is pure RFC&nbsp;6238 — no
    /// <c>SecurityStamp</c> dependency — so the code computed here is the same
    /// code the production verify endpoint will accept.
    /// </summary>
    private async Task<string> ComputeTotpCodeAsync(Guid userId)
    {
        ...
        HeimdallUser? user = await userManager.FindByIdAsync(userId.ToString("D")).ConfigureAwait(false);
        if (user is null)
        {
            throw new InvalidOperationException(
                $"Phase 4 acceptance: user {userId} disappeared between seed and TOTP compute.");
        }

        // The setup page GET already reset the key on the server; we do NOT
        // reset again here — that would invalidate the code against the live
        // pipeline. GenerateTwoFactorTokenAsync reads the stored key and
        // produces the same RFC 6238 30-second-window code the verify
        // endpoint computes.
        string? code = await userManager
            .GenerateTwoFactorTokenAsync(user, TokenOptions.DefaultAuthenticatorProvider)
            .ConfigureAwait(false);

        if (string.IsNullOrEmpty(code))
        {
            throw new InvalidOperationException(
                $"Phase 4 acceptance: GenerateTwoFactorTokenAsync returned no code for {userId}.");
        }
