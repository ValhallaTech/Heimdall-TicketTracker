...
-- Asserts the index is genuinely unique (not just present) so a future
-- migration cannot silently drop the uniqueness guarantee.
SELECT index_is_unique(
    'data_protection_keys',
    'ix_public_data_protection_keys_friendly_name'
);

-- Assert id is an identity column (pg_attribute.attidentity is 'a' for
-- GENERATED ALWAYS, 'd' for GENERATED BY DEFAULT). FluentMigrator emits
-- AsIdentity() which produces 'd'; either is acceptable, so we just require
-- attidentity to be non-empty.
SELECT isnt(
    (SELECT attidentity::text
       FROM pg_catalog.pg_attribute
      WHERE attrelid = 'public.data_protection_keys'::regclass
        AND attname = 'id'),
    '',
    'data_protection_keys.id is an identity column'
);
